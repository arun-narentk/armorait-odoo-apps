#!/usr/bin/env python3
"""Capture live Smart Color Tags screenshots and GIFs for Apps Store assets."""
...
partner = Partner.search([('name', '=', 'COLORTAG Capture Customer')], limit=1)
if not partner:
...
rule = Rule.search([('name', '=', 'COLORTAG Live Capture Rule')], limit=1)
if not rule:
    rule = Rule.with_context(rn_skip_color_recompute=True).create({
        'name': 'COLORTAG Live Capture Rule',
        'model_id': env.ref('sale.model_sale_order').id,
        'domain': "[('partner_id.name', 'ilike', 'COLORTAG%')]",
        'color': 'purple',
        'priority': 150,
        'icon': 'star',
        'label': 'CAPTURE',
...
out = {
    ...
    'rules_action_id': action_id('rn_smart_color_tags.action_rn_color_rule'),
...
def capture_assets(meta: dict) -> None:
    ...
    with sync_playwright() as p:
        ...
        hero_actions = [aid for aid in (rules_aid, sale_aid, invoice_aid) if aid]
        if hero_actions:
            record_action_sequence_gif(page, hero_actions, gifs_dir / 'hero.gif')
        if rules_aid and sale_aid:
            record_action_sequence_gif(page, [rules_aid, sale_aid], gifs_dir / 'workflow.gif')
...
def main() -> int:
    ...
    if not meta.get('rules_action_id'):
        raise SystemExit('Color rules action missing. Install rn_smart_color_tags first.')
    capture_assets(meta)
    subprocess.run(
